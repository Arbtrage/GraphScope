...
function EnvironmentsContent() {
  ...
  const [testConnection, { loading: testing }] = useGraphMutation(TEST_CONNECTION);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [sheetOpen, setSheetOpen] = useState(false);
  const [testFeedback, setTestFeedback] = useState<{ envId: string; ok: boolean; message: string } | null>(null);
  ...
  return (
    <div>
      <PageHeader
        title="Environments"
        description="Endpoints, headers, and secrets used when you run queries."
        action={
      ...
      {(data?.environments ?? []).length === 0 ? (
        <EmptyState title="No environments yet" description="Add an endpoint so Execute knows where to send queries." actionLabel="New environment" onAction={() => setSheetOpen(true)} />
      ) : (
                ...
                <TableRow key={env.id}>
                  <TableCell className="font-medium">
                    <div className="space-y-1">
                      <span>{env.name}</span>
                      {testFeedback?.envId === env.id && (
                        <p
                          role="alert"
                          className={`text-xs ${testFeedback.ok ? "text-[var(--success)]" : "text-destructive"}`}
                        >
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          const status = (result.data as { executeOperation?: { execution?: { status?: string } } } | null | undefined)
                            ?.executeOperation?.execution?.status;
                          const ok = status === "SUCCESS" || status === "OK";
                          setTestFeedback({
                            envId: env.id,
                            ok,
                            message: ok ? "Connection succeeded." : `Connection returned ${status ?? "unknown status"}.`,
                          });
                        } catch (err) {
                          setTestFeedback({
                            envId: env.id,
