...
function ProjectDetail() {
  ...
  const hasRepo = (data.repositoryLinks?.length ?? 0) > 0;
  const hasSchema = schemas.length > 0;
  const nextStep = !hasRepo
    ? {
        title: "Connect a repository",
        description: "Index GraphQL operations from your codebase.",
    ...
    <div>
        ...
        action={
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" size="sm" asChild>
              <Link href={`/app/schema/explore`}>Explore schema</Link>
            </Button>
            <Button
              size="sm"
              className="bg-execute text-execute-foreground hover:bg-execute/90"
              onClick={() => router.push("/app/execute")}
            >
              Run query
            </Button>
          </div>
      ...
      {nextStep && (
        <div className="mb-6 flex flex-col gap-3 rounded-lg border border-primary/25 bg-primary/5 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button size="sm" onClick={() => setTab(nextStep.tab)}>
            {nextStep.cta}
          </Button>
        </div>
      )}
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="repository" className="space-y-4" id="project-tab-repository">
        ...
        <TabsContent value="schema" className="space-y-4" id="project-tab-schema">
